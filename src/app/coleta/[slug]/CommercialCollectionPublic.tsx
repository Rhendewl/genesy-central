  ...
  return <main className="min-h-dvh bg-[#050607] bg-[radial-gradient(circle_at_12%_5%,rgba(176,184,192,.10),transparent_32%),radial-gradient(circle_at_90%_90%,rgba(96,106,112,.08),transparent_30%)] px-4 py-8 text-white sm:py-12">
    <div className="mx-auto max-w-2xl">
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[.045] p-5 shadow-[0_24px_80px_rgba(0,0,0,.42)] backdrop-blur-xl sm:p-8">
        ...
        <div className="min-h-[210px] overflow-hidden"><AnimatePresence mode="wait">{question && <motion.div key={`${development?.name}-${question.id}`} initial={{ opacity: 0, x: 22 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -22 }} transition={{ duration: 0.2 }}><Question question={question} value={answers[question.id]} onChange={(value) => setAnswers((current) => ({ ...current, [question.id]: value }))} onAutoAdvance={question.type === "rating" || question.type === "single_choice" ? answerAndAdvance : undefined} /></motion.div>}</AnimatePresence></div>
