...
export function CommercialCollectionPublic({ slug }: { slug: string }) {
  ...
  if (!identified) return <BrandShell><div className="w-full max-w-md"><div className="mb-7 text-center"><BrandMark /><h1 className="mt-6 text-2xl font-semibold">Antes de começar, quem é você?</h1><p className="mt-2 text-sm text-[#8d969c]">Selecione seu nome para vincular corretamente as respostas.</p></div><section className="rounded-3xl border border-white/10 bg-white/[.045] p-6 shadow-[0_28px_90px_rgba(0,0,0,.48)] backdrop-blur-xl"><label><span className="mb-2 block text-xs font-medium text-[#c3c9cd]">Corretor</span><select value={brokerId} onChange={(event) => setBrokerId(event.target.value)} className="w-full rounded-xl border border-white/10 bg-[#101214] px-3 py-3 text-sm outline-none transition focus:border-[#9ca7ad]"><option value="">Selecione seu nome</option>{brokers.map((broker) => <option key={broker.id} value={broker.id}>{broker.name}</option>)}</select></label><button disabled={!brokerId} onClick={() => { setIdentified(true); setShowDevelopmentIntro(true); }} className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#c3c9cd]/20 bg-[#b0b8c1] px-5 py-3 text-sm font-semibold text-[#08090a] shadow-[0_1px_10px_rgba(176,184,193,.10)] transition hover:bg-[#c3c9cd] active:bg-[#a4adb4] disabled:opacity-40">Continuar <ArrowRight size={16} /></button></section></div></BrandShell>;

  if (showDevelopmentIntro && development) return <BrandShell><motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-lg text-center"><BrandMark /><p className="mt-8 text-[10px] font-semibold uppercase tracking-[.28em] text-[#8d969c]">{developmentIndex === 0 ? "Vamos começar" : "Próxima campanha"}</p><h1 className="mt-5 text-2xl font-semibold leading-tight text-[#f2f3f4] sm:text-4xl">{developmentIndex === 0 ? <>Ok, vamos começar pela campanha do <span className="text-[#b8c0c5]">{development.name}</span>.</> : <>Agora você responderá sobre a campanha do <span className="text-[#b8c0c5]">{development.name}</span>.</>}</h1><p className="mx-auto mt-4 max-w-md text-sm leading-6 text-[#7f898f]">Responda às perguntas a seguir com base na sua experiência com este empreendimento.</p><div className="mx-auto mt-8 flex max-w-sm flex-col gap-3"><button type="button" onClick={() => setShowDevelopmentIntro(false)} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full border border-[#c3c9cd]/20 bg-[#b0b8c1] px-6 py-3 text-sm font-semibold text-[#08090a] shadow-[0_1px_10px_rgba(176,184,193,.10)] transition hover:bg-[#c3c9cd] active:bg-[#a4adb4]">Responder sobre este empreendimento <ArrowRight size={16} /></button><button type="button" onClick={skipDevelopment} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-full border border-rose-400/20 bg-gradient-to-r from-rose-950/70 via-rose-900/55 to-red-950/70 px-5 py-3 text-xs font-medium text-rose-100 shadow-[0_12px_34px_rgba(244,63,94,.10)] transition hover:border-rose-300/35 hover:brightness-110"><SkipForward size={14} />{developmentIndex + 1 >= collection.developments.length ? "Não participo desta campanha · Encerrar" : "Não participo desta campanha · Pular"}</button></div></motion.div></BrandShell>;
  ...
  return <main className="min-h-dvh bg-[#050607] bg-[radial-gradient(circle_at_12%_5%,rgba(176,184,192,.10),transparent_32%),radial-gradient(circle_at_90%_90%,rgba(96,106,112,.08),transparent_30%)] px-4 py-8 text-white sm:py-12">
    <div className="mx-auto max-w-2xl">
      <header className="mb-7"><BrandMark align="left" /><h1 className="mt-6 text-2xl font-semibold sm:text-3xl">{commercialPeriodTitle(collection.period_end)}</h1><p className="mt-2 text-sm text-[#8d969c]">{collection.clientName} · leva menos de 3 minutos</p></header>
...
function Question({ question, value, onChange, onAutoAdvance }: { question: FormStep; value: unknown; onChange: (value: unknown) => void; onAutoAdvance?: (value: unknown) => void }) {
  ...
  const longTextLength = typeof value === "string" ? value.trim().length : 0;
  return <div className="block"><h2 className="mb-2 text-2xl font-semibold leading-tight text-[#f2f3f4] sm:text-3xl">{question.title}{question.required && <span className="ml-1 text-[#aeb7bd]">*</span>}</h2>{question.description && <p className="mb-5 text-sm leading-6 text-[#8d969c]">{question.description}</p>}
    {question.type === "rating" && question.maxRating === 10 ? <div className="mt-7"><div className="flex flex-nowrap gap-1 sm:gap-2" role="radiogroup" aria-label="Nota de 0 a 10">{ratingValues.map((rating) => { const selected = currentRating === rating; const color = scoreColor(rating); return <div key={rating} className="relative min-w-0 flex-1">{selected && <motion.span aria-hidden="true" className="pointer-events-none absolute inset-0 rounded-xl" style={{ background: color, filter: "blur(9px)" }} initial={{ opacity: .35, scale: .9 }} animate={{ opacity: [.25, .68, .25], scale: [.92, 1.22, .92] }} transition={{ duration: 1.55, repeat: Infinity, ease: "easeInOut" }} />}<motion.button type="button" role="radio" aria-checked={selected} aria-label={`Nota ${rating}`} whileHover={{ scale: 1.08 }} whileTap={{ scale: .92 }} onClick={() => onAutoAdvance?.(rating)} className="relative flex aspect-square w-full items-center justify-center rounded-xl text-[11px] font-bold outline-none transition-colors focus-visible:ring-2 focus-visible:ring-white/50 sm:text-sm" style={{ background: selected ? color : "rgba(255,255,255,.045)", border: `1px solid ${selected ? color : "rgba(255,255,255,.11)"}`, color: selected ? "#fff" : "#aeb7bd" }}>{rating}</motion.button></div>; })}</div><div className="mt-4 h-1.5 rounded-full bg-gradient-to-r from-red-500 via-amber-400 to-emerald-400" /><div className="mt-2 flex justify-between text-[11px] text-[#7f898f]"><span>Baixa percepção</span><span>Alta percepção</span></div></div>
      ...
      : question.type === "long_text" ? <div className="mt-6"><textarea rows={5} value={String(value ?? "")} onChange={(event) => onChange(event.target.value)} placeholder={question.placeholder ?? "Conte com detalhes o que você percebeu..."} aria-describedby={`${question.id}-length-hint`} className="w-full resize-y rounded-xl border border-white/10 bg-[#101214]/90 px-4 py-4 text-base leading-6 outline-none backdrop-blur-xl transition placeholder:text-[#596168] focus:border-[#9ca7ad] focus:shadow-[0_0_36px_rgba(174,183,189,.08)]" /><div id={`${question.id}-length-hint`} className={`mt-2 flex items-start justify-between gap-4 text-[11px] ${longTextLength >= COMMERCIAL_LONG_TEXT_MIN_LENGTH ? "text-emerald-400" : "text-amber-300/80"}`}><span>{longTextLength >= COMMERCIAL_LONG_TEXT_MIN_LENGTH ? "Resposta completa." : "Conte um pouco mais para dar uma resposta completa."}</span><span className="shrink-0 tabular-nums">{longTextLength}/{COMMERCIAL_LONG_TEXT_MIN_LENGTH}</span></div></div>
...
function isAnswerValid(question: FormStep, answer: unknown) {
  return isCommercialAnswerValid(question, answer);
}

function commercialPeriodTitle(periodEnd?: string) {
  if (!periodEnd) return "Diagnóstico Comercial";
  const date = new Date(`${periodEnd}T12:00:00`);
  if (Number.isNaN(date.getTime())) return "Diagnóstico Comercial";
  const week = Math.min(5, Math.ceil(date.getDate() / 7));
  const month = date.toLocaleDateString("pt-BR", { month: "long" });
  const capitalizedMonth = month.charAt(0).toUpperCase() + month.slice(1);
  return `Diagnóstico Comercial | ${week}ª semana de ${capitalizedMonth}`;
}
