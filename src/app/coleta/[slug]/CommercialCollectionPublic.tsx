...
export function CommercialCollectionPublic({ slug }: { slug: string }) {
  ...
  if (showDevelopmentIntro && development) return <BrandShell><motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-lg text-center"><p className="text-[10px] font-semibold uppercase tracking-[.28em] text-[#8d969c]">Próxima campanha</p><h1 className="mt-5 text-2xl font-semibold leading-tight text-[#f2f3f4] sm:text-4xl">Perfeito, agora vou te perguntar sobre a campanha do <span className="text-[#b8c0c5]">{development.name}</span>.</h1><button type="button" onClick={() => setShowDevelopmentIntro(false)} className="mx-auto mt-8 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#7b868d] to-[#b0b8c0] px-6 py-3 text-sm font-semibold text-[#08090a] shadow-[0_12px_34px_rgba(160,170,176,.16)] transition hover:brightness-110">Ok, vamos lá <ArrowRight size={16} /></button></motion.div></BrandShell>;
