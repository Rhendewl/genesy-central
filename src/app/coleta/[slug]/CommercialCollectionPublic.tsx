...
export function CommercialCollectionPublic({ slug }: { slug: string }) {
  ...
  const valid = useMemo(() => collection?.questions.every((question) => !question.required || (answers[question.id] !== undefined && answers[question.id] !== "")), [answers, collection]);
  ...
  return <main className="min-h-dvh bg-[#071017] px-4 py-8 text-white sm:py-12">
    <div className="mx-auto max-w-2xl">
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[.055] p-5 shadow-[0_24px_80px_rgba(0,0,0,.35)] backdrop-blur-xl sm:p-8">
        <div className="mb-7 grid gap-4 sm:grid-cols-2"><label><span className="mb-2 block text-xs font-medium text-slate-300">Corretor</span><select value={brokerId} onChange={(event) => setBrokerId(event.target.value)} className="w-full rounded-xl border border-white/10 bg-[#0d1821] px-3 py-3 text-sm outline-none focus:border-sky-500"><option value="">Selecione seu nome</option>{brokers.map((broker) => <option key={broker.id} value={broker.id}>{broker.name}</option>)}</select></label><div><span className="mb-2 block text-xs font-medium text-slate-300">Empreendimento</span><div className="rounded-xl border border-sky-400/20 bg-sky-400/10 px-3 py-3 text-sm font-semibold text-sky-200">{development?.name}</div></div></div>
...
function Question({ question, value, onChange }: { question: FormStep; value: unknown; onChange: (value: unknown) => void }) {
  return <label className="block"><span className="mb-2 block text-sm font-medium text-slate-100">{question.title}{question.required && <span className="ml-1 text-sky-400">*</span>}</span>{question.description && <span className="mb-2 block text-xs text-slate-400">{question.description}</span>}
    {question.type === "rating" ? <div className="flex flex-wrap gap-2">{Array.from({ length: question.maxRating ?? 5 }, (_, index) => index + 1).map((rating) => <button key={rating} type="button" onClick={() => onChange(rating)} className={`grid h-10 w-10 place-items-center rounded-xl border text-sm transition ${value === rating ? "border-sky-400 bg-sky-400/20 text-sky-200" : "border-white/10 bg-white/5 text-slate-400 hover:border-white/25"}`}>{rating === (question.maxRating ?? 5) ? <Star size={14} fill="currentColor" /> : rating}</button>)}</div>
      : question.type === "single_choice" || question.type === "multiple_choice" ? <div className="grid gap-2 sm:grid-cols-2">{question.choices?.map((choice) => <button key={choice.id} type="button" onClick={() => onChange(choice.value)} className={`rounded-xl border px-3 py-3 text-left text-sm transition ${value === choice.value ? "border-sky-400 bg-sky-400/15 text-sky-100" : "border-white/10 bg-white/[.03] text-slate-300 hover:border-white/25"}`}>{choice.label}</button>)}</div>
