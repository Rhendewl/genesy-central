        ...
        {isLoading ? <MarketingSkeleton /> : error ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
              <MetricCard label={periodMode === "month" ? "VGV no mês" : "VGV no período"} value={formatCurrency(metrics.total)} icon={<TrendingUp />} accent="#27a3ff" />
              ...
              <MetricCard label="Comissão estimada" value={formatCurrency(metrics.commissions)} icon={<HandCoins />} accent="#22c55e" />
            </div>
                      ...
                      {sales.map((sale) => (
                        <tr key={sale.id} className="border-t transition-colors hover:bg-[var(--hover)]" style={{ borderColor: "var(--border)" }}>
                          ...
                          <td className="whitespace-nowrap px-5 py-3.5 text-right">
                            ...
                            <span className="ml-2 text-xs text-[var(--muted-foreground)]">{formatCurrency(sale.sale_value * sale.commission_percentage / 100)}</span>
                          </td>
