...
function DiagnosisCard({ analysis, onOpen, onEdit, onDelete }: { analysis: CommercialAnalysis; onOpen: () => void; onEdit: () => void; onDelete: () => void }) {
  return <article onClick={onOpen} className="glass-folder-card group relative flex aspect-square cursor-pointer flex-col items-center justify-center overflow-hidden rounded-[20px] border p-3 text-center transition-all hover:-translate-y-1" style={{ "--folder-color": "#6f8493" } as React.CSSProperties}>
    ...
    <p className="relative mt-3 w-full truncate text-sm font-semibold text-[var(--text-title)]">{formatCommercialAnalysisTitle(analysis.meeting_date)}</p>
...
function AnalysisFormModal({ open, clientId, analysis, onClose, onSaved }: { open: boolean; clientId: string; analysis: CommercialAnalysis | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return createPortal(<div className="lc-modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}><form onSubmit={submit} className="lc-modal-panel flex h-[calc(100dvh-1.5rem)] w-full max-w-4xl flex-col overflow-hidden rounded-2xl text-[var(--text-title)] sm:h-[min(90dvh,800px)]"><div className="shrink-0 flex items-center justify-between border-b border-[var(--border)] px-5 py-4"><div><h2 className="text-sm font-semibold text-[var(--text-title)]">{analysis ? "Editar análise comercial" : "Nova análise comercial"}</h2><p className="text-[11px] text-[var(--muted-foreground)]">Os indicadores serão calculados e diagnosticados automaticamente.</p></div><button type="button" onClick={onClose} className="rounded-lg p-1.5 text-[var(--muted-foreground)] transition-colors hover:bg-[var(--hover)] hover:text-[var(--text-title)]"><X size={16} /></button></div><div className="min-h-0 flex-1 space-y-6 overflow-y-auto overscroll-contain p-5">
    ...
    <FormSection title="3. Perfil dos leads"><div className="grid gap-3 sm:grid-cols-5"><Input label="Qualificados" type="number" value={String(data.qualified_leads)} onChange={(v) => patch("qualified_leads", Number(v))} /><Input label="Desqualificados" type="number" value={String(data.disqualified_leads)} onChange={(v) => patch("disqualified_leads", Number(v))} /><Input label="Quentes" type="number" value={String(data.hot_leads)} onChange={(v) => patch("hot_leads", Number(v))} /><Input label="Mornos" type="number" value={String(data.warm_leads)} onChange={(v) => patch("warm_leads", Number(v))} /><Input label="Frios" type="number" value={String(data.cold_leads)} onChange={(v) => patch("cold_leads", Number(v))} /></div><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="block"><span className="lc-form-label mb-1.5 block text-xs font-medium">Tipologia do produto</span><select value={data.product_type} onChange={(e) => patch("product_type", e.target.value as CommercialProductType)} className="lc-form-control crm-form-select rounded-lg px-3 py-2 text-sm">{Object.entries(COMMERCIAL_PRODUCT_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><Input label="Nome do empreendimento" value={data.development_name ?? ""} onChange={(v) => patch("development_name", v || null)} /></div><TextArea label="Análise qualitativa do perfil dos leads" value={data.lead_profile_notes} onChange={(v) => patch("lead_profile_notes", v)} /></FormSection>
  ...
  </div><div className="shrink-0 flex justify-end gap-2 border-t border-[var(--border)] px-5 py-4"><button type="button" onClick={onClose} className="rounded-lg border border-[var(--border)] px-4 py-2 text-xs text-[var(--muted-foreground)] transition-colors hover:bg-[var(--hover)] hover:text-[var(--text-title)]">Cancelar</button><button disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-[var(--primary)] px-4 py-2 text-xs font-semibold text-[var(--primary-foreground)] disabled:opacity-50">{saving && <Loader2 size={12} className="animate-spin" />} Salvar e gerar diagnóstico</button></div></form></div>, document.body);
}
...
function Input({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (value: string) => void; type?: string }) { return <label className="block"><span className="lc-form-label mb-1.5 block text-xs font-medium">{label}</span><input type={type} min={type === "number" ? 0 : undefined} value={value} onChange={(e) => onChange(e.target.value)} className="lc-form-control rounded-lg px-3 py-2 text-sm" /></label>; }
