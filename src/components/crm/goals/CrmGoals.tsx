...
function GoalModal({ open, goal, pipelines, onClose, onSaved }: { open: boolean; goal: CrmGoal | null; pipelines: ReturnType<typeof usePipelines>["pipelines"]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="lc-modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,.65)" }} onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <form onSubmit={submit} className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border" style={{ background: "var(--card)" }}>
      <div className="flex items-center justify-between border-b px-5 py-4"><div><h2 className="text-sm font-semibold">{goal ? "Editar meta" : "Nova meta"}</h2><p className="text-[11px] text-[var(--muted-foreground)]">Preencha a receita e, se quiser, fixe também metas de volume.</p></div><button type="button" onClick={onClose} className="rounded-lg p-1.5 hover:bg-[var(--hover)]"><X size={15} /></button></div>
      <div className="space-y-4 p-5">
        <Field label="Nome"><input required value={name} onChange={(e) => setName(e.target.value)} className="lc-input w-full rounded-lg px-3 py-2 text-sm" /></Field>
        <Field label="Pipeline"><select value={pipelineId} onChange={(e) => setPipelineId(e.target.value)} className="lc-input w-full rounded-lg px-3 py-2 text-sm"><option value="">Todas as pipelines</option>{pipelines.map((pipeline) => <option key={pipeline.id} value={pipeline.id}>{pipeline.name}</option>)}</select></Field>
        <div className="grid grid-cols-2 gap-3"><Field label="Início"><input required type="date" value={startsAt} onChange={(e) => setStartsAt(e.target.value)} className="lc-input w-full rounded-lg px-3 py-2 text-sm" /></Field><Field label="Fim"><input required type="date" min={startsAt} value={endsAt} onChange={(e) => setEndsAt(e.target.value)} className="lc-input w-full rounded-lg px-3 py-2 text-sm" /></Field></div>
        <Field label="Meta de receita (R$)"><input type="number" min="0" step="0.01" value={targets.revenue_target} onChange={(e) => setTarget("revenue_target", e.target.value)} placeholder="Ex: 100000" className="lc-input w-full rounded-lg px-3 py-2 text-sm" /></Field>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><Field label="Vendas"><input type="number" min="0" value={targets.sales_target} onChange={(e) => setTarget("sales_target", e.target.value)} placeholder="Automático" className="lc-input w-full rounded-lg px-3 py-2 text-sm" /></Field><Field label="Comparecimentos"><input type="number" min="0" value={targets.held_meetings_target} onChange={(e) => setTarget("held_meetings_target", e.target.value)} placeholder="Automático" className="lc-input w-full rounded-lg px-3 py-2 text-sm" /></Field><Field label="Agendamentos"><input type="number" min="0" value={targets.scheduled_meetings_target} onChange={(e) => setTarget("scheduled_meetings_target", e.target.value)} placeholder="Automático" className="lc-input w-full rounded-lg px-3 py-2 text-sm" /></Field></div>
      ...
      <div className="flex justify-end gap-2 border-t px-5 py-4"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2 text-xs">Cancelar</button><button disabled={saving || !name.trim()} className="inline-flex items-center gap-2 rounded-lg bg-[var(--primary)] px-4 py-2 text-xs font-semibold text-white disabled:opacity-50">{saving && <Loader2 size={12} className="animate-spin" />} Salvar meta</button></div>
    </form>
  </div>;
}
